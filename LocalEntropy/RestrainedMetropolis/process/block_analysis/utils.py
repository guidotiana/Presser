...
def load_data(d, return_eq = True):
    datafiles = np.array([f for f in listdir(f'{d}') if isfile(f'{d}/{f}') and ('data' in f) and ('.dat' in f)])
...
def load_muts(d, return_eq = True, return_pars = True):
    mutsfiles = np.array([f for f in listdir(f'{d}') if isfile(f'{d}/{f}') and ('mutants' in f) and ('.dat' in f)])
