...
class Basic_class:
    ...
    def _check_Ca(self, plddt_mask, sequence):
        ...
        assert check, r'Missing C-$\alpha$ for loaded sequence: ' + f'{sequence}'
...
class Mutation_class(Basic_class):
    ...
    def _restart(self):
        ...
        if check:
            ...
            self.max_distance = float( last_line[6] )
            self.T = float( last_line[-3] )
            self.k = float( last_line[-2] )
    ...
    def metropolis(self, equilibration = False, print_start = True):
        ...
        for imut in range(mutations):
            ...
            # Update lists
            weight = (eff_energy - self.last_eff_energy) / self.T
